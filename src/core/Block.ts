...
export default class Block<P extends object = {}> {
  ...
  validate(element: HTMLInputElement) {
    ...
    const { value } = element;
    const id = element.id as keyof typeof REG;
    const errorEl = element.nextElementSibling!;

    if (!value) {
      errorEl.textContent = ERRORS.empty;
    } else if (REG[id] && !REG[id].test(value)) {
      errorEl.textContent = ERRORS[id as keyof typeof ERRORS];
    }

    if (element.matches('#password2')) {
      const password = (this.refs.password.children[1] as HTMLInputElement)
        .value;
      if (value !== password) {
        errorEl.textContent = ERRORS.passwordNotMatch;
      }
    }
  }
}
